...
namespace WebApp.Services
{
    public class InputValidationService
    {
        ...
        public async Task<bool> ValidateDate(string input)
        {
            ...
            else if (Regex.IsMatch(input, @"^(0?[1-9]|1[0-2])\/(0?[1-9]|[12]\d|3[01])\/\d{4} (0?[1-9]|1[0-2]):[0-5]\d:[0-5]\d$"))
            {
                return true;
            }
            else if (Regex.IsMatch(input, @"^(0?[1-9]|1[0-2])\/(0?[1-9]|[12]\d|3[01])\/\d{4} (0?\d|1\d|2[0-3]):[0-5]\d:[0-5]\d$"))
            {
                return true;
            }
            else
